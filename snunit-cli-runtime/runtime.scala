...
private inline def handlerImpl(
    req: Request,
    code: StatusCode,
    res: String,
    contentType: String
...
private inline def textPlainResponse(
    req: Request,
    code: StatusCode,
    res: String
): Unit = handlerImpl(req, code, res, "text/plain")

private transparent inline def toWritable[T](
    req: Request,
    code: StatusCode,
    t: T
): Unit = inline t match {
...
private transparent inline def wrapHandler[T](handler: T): Handler =
